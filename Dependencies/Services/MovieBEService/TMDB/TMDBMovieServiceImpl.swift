...
extension MovieServiceImplementation {
    ...
    public func updateItemWhenVisible(model: MovieModel, delegate: MovieServiceBrowseItemDelegate) {
        
        if let tmdbModel = movieModel(id: model.id), let posterPath = tmdbModel.posterPath {
            ...
            posterImagesCancellables[model.id] = api.mediaObject(path:posterPath, size:200, type:.JPG)
                .sink(receiveCompletion: { _ in }, receiveValue: { [weak self] value in
                    ...
                    delegate.updateMovie(model: model)
                    self.posterImagesCancellables[model.id] = nil
                })
        }
    }
...
extension MovieServiceImplementation {
    ...
    private func fetchMovies() {
        ...
        switch list {
            case .popular: cancellable = api.MovieBrowser(page:page)
                .receive(on: RunLoop.main)
                .sink(receiveCompletion: receiveCompletion, receiveValue: receiveValue)
            case .topRated: cancellable = api.topRatedMovies(page:page)
                .receive(on: RunLoop.main)
                .sink(receiveCompletion: receiveCompletion, receiveValue: receiveValue)
            case .upComing: cancellable = api.upComingMovies(page:page)
                .receive(on: RunLoop.main)
                .sink(receiveCompletion: receiveCompletion, receiveValue: receiveValue)
        }
    ...
    private func sendNewMovies() {
        ...
        func genremap(ids:[Int]) -> [MovieGenre] {
...
extension MovieServiceImplementation {
    ...
    func movieModel(id: Int) -> TMDBMovieModel? {
